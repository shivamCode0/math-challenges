...
function AMCPractice({ filterText, comp }) {
  ...
  const [batch, setBatch] = useLocalStorage<AMCProblem[]>(`mathchallenges_${comp}_batch1`, undefined);
  ...
  useEffect(() => {
    if (batch === null) {
      ...
      fetch(`/api/amcproblems?q=${filterText}&n=5`)
        .then((res) => res.json())
        .then((data) => {
          // console.log(data);
          setBatch(data);
